...
const index = () => {
  ...
  const TextCompanent = ({ detail,index }: { detail: string,index:number }) => {
    return (
      <View
        style={{
          paddingVertical: 12,
      ...
      >
        <Text style={{color:index ===0 ? '#4e4e4e':'#687482',fontSize:index ===0 ? 10.5 : 13,fontWeight:index === 0 ?'400' : '500'}}>{detail}</Text>
